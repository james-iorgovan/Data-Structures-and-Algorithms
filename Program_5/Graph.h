...
class graph {
  ...
  public:
    ...
    bool valid_topo_order(const vector<int> & order) {
      if(has_cycle())
        return false;
		vector<bool> visitedArr = vector<bool>(num_nodes());
		int size = visitedArr.size();
		...
		if(visitedArr[vertices[u].incoming[j].vertex_id] == false){
		return false;
		}
	   }
	  }
	...
	//condition to make sure that there is no extra spacing
	if(str == ""){
	str = id2name(i);
	}else{
	str = id2name(i) + " " + str;
